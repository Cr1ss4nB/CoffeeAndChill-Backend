...
def process_checkout(session: Session, checkout_data: CheckoutRequest, current_user: UserResponse) -> Order:
    """
    ...
    # Since System User ID is mandatory per schema for "Order":
    # If the user is a client, we assign a dummy system user or assign the order to admin 
    # as the DBML requires `system_user_id` as non-nullable. Usually an employee accepts the order.
    # For now, we will assign system_user_id = 1 (System / Admin) for online orders.
    system_user_id = 1 # Fallback system user for auto-processing
    ...
    # Attach order ID to items and save
